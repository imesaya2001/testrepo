...
class PayHere_Admin
{
    ...
    {
        $gateWay = new WC_Gateway_PayHere();
    ...
    public function register_authorized_order_status()
    {
        register_post_status('wc-payhere-authorized', array(
            ...
            'label_count' => _n_noop('Authorized by PayHere (%s)', 'Authorized by PayHere (%s)')
        ));
    }
    ...
    public function add_authorized_to_order_statuses($order_statuses)
    {
        ...
        foreach ($order_statuses as $key => $status) {
            ...
            if ('wc-processing' === $key) {
                $new_order_statuses['wc-phauthorized'] = 'Authorized by PayHere';
            }
        }
    ...
    {
        if($status == false && $order->get_status() =='phauthorized'){
            return true;
        }
        return $status;
    }

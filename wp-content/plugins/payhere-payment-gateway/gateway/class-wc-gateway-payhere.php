...
//
    ...
    {
        return $this->gatewayUtilities->generate_info_box_html($key, $data);
    }
     ...
     **/
    private function generate_payhere_form($order_id)
    {
        global $woocommerce;
...
//        Frontend Template Data
        ...
        ob_start();
        if ($this->payment_action == 'authorization' &&  !isset($payhere_args['recurrence']) ) {
            if($onsite_checkout_enabled){
                $payhere_js_data['payhere_args']['authorize'] = true;
                $payhere_js_data['payhere_obj']['authorize'] = true;
            }
            wp_enqueue_script('payhere-checkout', plugin_dir_url(__FILE__) . '../public/js/payhere-checkout-auth.js', array('jquery'), '2.0.0', false);
            ...
            wp_enqueue_script('payhere-checkout', plugin_dir_url(__FILE__) . '../public/js/payhere-checkout-sale.js', array('jquery'), '2.0.0', false);
            wp_localize_script('payhere-checkout', 'payhere_config', $payhere_js_data);
    ...
    public function check_payhere_response()
    {
        ...
        $gateway_utilities = new GatewayUtilities();
        $gateway_utilities->_log('REQUEST', $_POST);

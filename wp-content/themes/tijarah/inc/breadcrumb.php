...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the tijarah_Breadcrumb class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to tijarah_Breadcrumb.
 * @return void
 */
function tijarah_breadcrumb( $args = array() ) {

  $breadcrumb = apply_filters( 'tijarah_Breadcrumb_object', null, $args );

  if ( ! is_object( $breadcrumb ) )
    $breadcrumb = new tijarah_Breadcrumb( $args );
...
class tijarah_Breadcrumb {
  ...
  /**
   * Sets up the breadcrumb trail properties.  Calls the `tijarah_Breadcrumb::add_items()` method
   ...
   *     @type array     $labels         Text labels. @see tijarah_Breadcrumb::set_labels()
   *     @type array     $post_taxonomy  Taxonomies to use for post types. @see tijarah_Breadcrumb::set_post_taxonomy()
  ...
  public function __construct( $args = array() ) {
    ...
    $this->args = apply_filters( 'tijarah_Breadcrumb_args', wp_parse_args( $args, $defaults ) );
  ...
  public function trail() {
    ...
    if ( 0 < $item_count ) {
      ...
      $breadcrumb = sprintf(
        '<%1$s role="navigation" aria-label="%2$s" class="tijarah breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
    ...
    $breadcrumb = apply_filters( 'tijarah_Breadcrumb', $breadcrumb, $this->args );
  ...
  protected function set_labels() {
    ...
    $defaults = array(
      'browse'              => esc_html__( 'Browse:',                               'tijarah' ),
      'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'tijarah' ),
      'home'                => esc_html__( 'Home',                                  'tijarah' ),
      'error_404'           => esc_html__( '404 Not Found',                         'tijarah' ),
      'archives'            => esc_html__( 'Archives',                              'tijarah' ),
      // Translators: %s is the search query.
      'search'              => esc_html__( 'Search results for: %s',                'tijarah' ),
      // Translators: %s is the page number.
      'paged'               => esc_html__( 'Page %s',                               'tijarah' ),
      // Translators: %s is the page number.
      'paged_comments'      => esc_html__( 'Comment Page %s',                       'tijarah' ),
      // Translators: Minute archive title. %s is the minute time format.
      'archive_minute'      => esc_html__( 'Minute %s',                             'tijarah' ),
      // Translators: Weekly archive title. %s is the week date format.
      'archive_week'        => esc_html__( 'Week %s',                               'tijarah' ),
    ...
    $this->labels = apply_filters( 'tijarah_Breadcrumb_labels', wp_parse_args( $this->args['labels'], $defaults ) );
  }
  ...
  protected function set_post_taxonomy() {
    ...
    $this->post_taxonomy = apply_filters( 'tijarah_Breadcrumb_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
  }
  ...
  protected function add_items() {
    ...
    $this->items = array_unique( apply_filters( 'tijarah_Breadcrumb_items', $this->items, $this->args ) );
  }
  ...
  protected function add_minute_hour_archive_items() {
    ...
    if ( true === $this->args['show_title'] )
      $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'tijarah' ) ) );
  }
  ...
  protected function add_minute_archive_items() {
    ...
    // Add the minute item.
    if ( true === $this->args['show_title'] )
      $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'tijarah' ) ) );
  }
  ...
  protected function add_hour_archive_items() {
    ...
    // Add the hour item.
    if ( true === $this->args['show_title'] )
      $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'tijarah' ) ) );
  }
  ...
  protected function add_day_archive_items() {
    ...
    $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'tijarah' ) ) );
    $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'tijarah' ) ) );
    $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'tijarah' ) ) );
  ...
  protected function add_week_archive_items() {
    ...
    $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'tijarah' ) ) );
    $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'tijarah' ) ) );
  ...
  protected function map_rewrite_tags( $post_id, $path ) {
    ...
    if ( is_array( $matches ) ) {
      ...
      foreach ( $matches as $match ) {
        ...
        if ( '%year%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'tijarah' ) ) ) );

        // If using the %monthnum% tag, add a link to the monthly archive.
        elseif ( '%monthnum%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'tijarah' ) ) ) );

        // If using the %day% tag, add a link to the daily archive.
        elseif ( '%day%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'tijarah' ) ) ) );
